"""A2A server for the Market Analyst sub-agent."""
...
from agents import market_analyst

AGENT_URL = os.getenv("MARKET_ANALYST_URL", "http://localhost:8080")
...
def build_app() -> Starlette:
    card = AgentCard(
        name="Market Analyst",
        description="Analisi quantitativa mercati Polymarket (volume, z-score, price history)",
        version="1.0.0",
        skills=[AgentSkill(id="market-signals", name="Market Signals")],
    ...
    handler = DefaultRequestHandler(
        agent_card=card,
        agent_executor=MarketAnalystExecutor(),
