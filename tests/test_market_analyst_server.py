...
async def test_agent_card_served():
    ...
    os.environ.setdefault("MARKET_ANALYST_URL", "http://localhost:8080")
    with (
        patch("agents.market_analyst.anthropic.Anthropic"),
        patch("tools.db.get_conn"),
    ):
        from agents.market_analyst_server import build_app
    ...
    data = response.json()
    assert data["name"] == "Market Analyst"
    assert any(s["id"] == "market-signals" for s in data["skills"])


@pytest.mark.asyncio
async def test_executor_calls_run_with_payload():
    """Executor extracts the data part and calls market_analyst.run() with the correct payload."""
    with (
        patch("agents.market_analyst.anthropic.Anthropic"),
        patch("tools.db.get_conn"),
    ):
        from agents.market_analyst_server import MarketAnalystExecutor
    ...
    context.message = Message(
        role=Role.ROLE_USER,
        message_id="m1",
        parts=[new_data_part({"hours": 24, "zscore_threshold": 2.0})],
    )
    queue = AsyncMock()

    with patch("agents.market_analyst.run", return_value={"signals": []}) as mock_run:
        executor = MarketAnalystExecutor()
        await executor.execute(context, queue)

    mock_run.assert_called_once_with(
        {"hours": 24, "zscore_threshold": 2.0}, langfuse_trace_id=None, langfuse_obs_id=None
    )
...
async def test_executor_emits_single_message():
    ...
    with patch("agents.market_analyst.run", return_value={"signals": [{"market_id": "m1"}]}):
        executor = MarketAnalystExecutor()
        await executor.execute(context, queue)
