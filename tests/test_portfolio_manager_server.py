...
async def test_agent_card_served():
    ...
    os.environ.setdefault("PORTFOLIO_MANAGER_URL", "http://localhost:8080")
    with (
        patch("agents.portfolio_manager.anthropic.Anthropic"),
        patch("tools.db.get_conn"),
    ):
        from agents.portfolio_manager_server import build_app
    ...
    data = response.json()
    assert data["name"] == "Portfolio Manager"
    assert any(s["id"] == "portfolio-state" for s in data["skills"])


@pytest.mark.asyncio
async def test_executor_calls_run_with_no_args():
    """Executor calls portfolio_manager.run() with NO arguments, ignoring payload."""
    with (
        patch("agents.portfolio_manager.anthropic.Anthropic"),
        patch("tools.db.get_conn"),
    ):
        from agents.portfolio_manager_server import PortfolioManagerExecutor
    ...
    context.message = Message(
        role=Role.ROLE_USER,
        message_id="m1",
        parts=[new_data_part({"some": "payload"})],
    )
    queue = AsyncMock()

    with patch("agents.portfolio_manager.run", return_value={"cash_available": 9000}) as mock_run:
        executor = PortfolioManagerExecutor()
...
async def test_executor_emits_single_message():
    ...
    with patch("agents.portfolio_manager.run", return_value={"cash_available": 9000}):
        executor = PortfolioManagerExecutor()
        await executor.execute(context, queue)
...
async def test_executor_handles_empty_parts():
    """Executor does not crash when message has no parts (Portfolio Manager ignores payload)."""
    with (
        patch("agents.portfolio_manager.anthropic.Anthropic"),
        patch("tools.db.get_conn"),
    ):
