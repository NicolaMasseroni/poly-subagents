...
async def test_agent_card_served():
    ...
    os.environ.setdefault("TRADER_URL", "http://localhost:8080")
    with (
        patch("agents.trader.anthropic.Anthropic"),
        patch("tools.db.get_conn"),
    ):
        from agents.trader_server import build_app
    ...
    data = response.json()
    assert data["name"] == "Trader"
    assert any(s["id"] == "trade-execution" for s in data["skills"])


@pytest.mark.asyncio
async def test_executor_calls_run_with_bets_and_cycle():
    """Executor extracts bets and cycle from payload and calls trader.run() correctly."""
    with (
        patch("agents.trader.anthropic.Anthropic"),
        patch("tools.db.get_conn"),
    ):
        from agents.trader_server import TraderExecutor
    ...
    with patch("agents.trader.run", return_value={"executions": []}) as mock_run:
        executor = TraderExecutor()
        await executor.execute(context, queue)

    mock_run.assert_called_once_with(
        [{"market_id": "m1", "outcome": "YES", "stake": 50, "odds": 1.8}],
        cycle=3,
...
async def test_executor_emits_single_message():
    ...
    context.message = Message(
        role=Role.ROLE_USER,
        message_id="m1",
        parts=[new_data_part({"bets": [], "cycle": 1})],
    )
    queue = AsyncMock()

    with patch("agents.trader.run", return_value={"executions": [{"market_id": "m1"}]}):
        executor = TraderExecutor()
        await executor.execute(context, queue)
...
async def test_executor_handles_empty_parts():
    """Executor falls back to empty bets and cycle=0 when message has no parts."""
    with (
        patch("agents.trader.anthropic.Anthropic"),
        patch("tools.db.get_conn"),
    ):
    ...
    mock_run.assert_called_once_with([], cycle=0, langfuse_trace_id=None, langfuse_obs_id=None)
